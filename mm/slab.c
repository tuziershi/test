		...
		gfp_t flags, int nodeid, struct page *page)
{
	...
	{
		printk(KERN_INFO "need to modify pte\n");
	}
...
static void *cache_alloc_refill(struct kmem_cache *cachep, gfp_t flags,
							bool force_refill)
{
	printk(KERN_INFO "cache_alloc_refill\n");
	int batchcount;
	struct kmem_cache_node *n;
	struct array_cache *ac;
	int node;
...
{
	printk(KERN_INFO "____cache_alloc\n");
	void *objp;
	struct array_cache *ac;
	...
	for (i = 0; i < nr_objects; i++) {
		...
		if (page->active == 0) {
			if (n->free_objects > n->free_limit) {
				 ...
				 * alloc_slabmgmt.
				 */
				int size=(1<<compound_order(page))*PAGE_SIZE;
					  ...
					  unsigned long caller)
{
	 ...
	 */
	cachep = kmalloc_slab(size, flags);
	printk(KERN_INFO "cache name:%s %d\n",cachep->name,flags&__GFP_COME_FROM_FILESYSTEM);
	if (unlikely(ZERO_OR_NULL_PTR(cachep)))
		return cachep;
